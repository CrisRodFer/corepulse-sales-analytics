CREATE OR REPLACE VIEW `corepulse-sales-analytics.corepulse_analytics.vw_ventas_base` AS
...
FROM `corepulse-sales-analytics.corepulse_dw.fact_ventas` f

LEFT JOIN `corepulse-sales-analytics.corepulse_dw.dim_producto` p
    ON f.product_id = p.product_id

LEFT JOIN `corepulse-sales-analytics.corepulse_dw.dim_categoria` c
    ON f.category_id = c.category_id

LEFT JOIN `corepulse-sales-analytics.corepulse_dw.dim_proveedor` pr
    ON f.provider_id = pr.provider_id

LEFT JOIN `corepulse-sales-analytics.corepulse_dw.dim_calendario` cal
    ON f.year_week_key = cal.year_week_key;


CREATE OR REPLACE VIEW `corepulse-sales-analytics.corepulse_analytics.vw_ranking_productos_anual_escenario` AS
...
CREATE OR REPLACE VIEW `corepulse-sales-analytics.corepulse_analytics.vw_ranking_proveedores_anual_escenario` AS
...
CREATE OR REPLACE VIEW `corepulse-sales-analytics.corepulse_analytics.vw_comparativa_ranking_productos_anual` AS
...
SELECT
    *,
    ...
    CAST(year_anterior AS STRING)
        || ' '
        || escenario_anterior
        || ' vs '
        || CAST(year_actual AS STRING)
        || ' '
...
CREATE OR REPLACE VIEW `corepulse-sales-analytics.corepulse_analytics.vw_comparativa_ranking_proveedores_anual` AS
...
CREATE OR REPLACE VIEW `corepulse-sales-analytics.corepulse_analytics.vw_ventas_yoy_producto_semana` AS
...
CREATE OR REPLACE VIEW `corepulse-sales-analytics.corepulse_analytics.vw_ventas_yoy_categoria_semana` AS
